...
import {
  ...
  EssentialSignupData,
  essentialSignupSchema,
  verificationAccountSchema,
} from "@/types/userFormSchema";
import { useTranslations } from "next-intl";

interface SignupEssentialProps {
  accountType?: 'personal' | 'business';
}

const SignupEssential = ({ accountType: initialAccountType = 'personal' }: SignupEssentialProps) => {
  ...
  const [formData, setFormData] = useState<Partial<EssentialSignupData>>({});
  const [userId, setUserId] = useState<string | null>(null);
  const [verificationComplete, setVerificationComplete] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const [showPasswordRequirements, setShowPasswordRequirements] = useState(false);

  const form = useForm<EssentialSignupData>({
    resolver: zodResolver(essentialSignupSchema),
    defaultValues: {
      firstName: "",
      lastName: "",
      email: "",
  ...
  // Session storage key for draft recovery
    ...
    if (typeof window !== "undefined") {
      ...
      if (savedDraft) {
        try {
          ...
          const { password, ...safeData } = draftData;
          form.reset(safeData);
          toast({
            title: t("draftRecovered") || "Draft recovered",
            description: t("draftRecoveredDescription") || "Your previous form data has been restored",
          });
        } catch (error) {
  ...
  // Save draft to session storage on form changes
  useEffect(() => {
    if (typeof window !== "undefined") {
      const subscription = form.watch((formData) => {
        // Don't save if form is empty or just default values
  ...
  useEffect(() => {
    if (password) {
      ...
      setPasswordStrength({
        ...
        meetsAllRequirements: metRequirements.length === PASSWORD_REQUIREMENTS.length,
      });
    }
  }, [password]);
  ...
  const handleStep1Submit = async (data: EssentialSignupData) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!response.success || !response.data) {
        ...
        if (response.error?.status === 409) {
          // User already exists - provide helpful message
      ...
      // Clear session storage draft after successful registration
      if (typeof window !== "undefined") {
                    ...
                    <CardContent className="space-y-6">
                      <Form {...form}>
                        <form
                          ...
                          className="space-y-5"
                        >
                          {/* Account Type - Hidden (determined by route) */}
                                ...
                                render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormDescription className="text-xs">
                                      Required for commercial shipments over €150
                                    </FormDescription>
                                    <FormMessage className="text-xs mt-1.5" />
                                  </FormItem>
                                )}
              ...
              >
                <div className="space-y-2">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 max-w-md mx-auto">
                    Your account has been successfully created. Get 15% off your
                    first shipment!
                  </p>
                </div>
